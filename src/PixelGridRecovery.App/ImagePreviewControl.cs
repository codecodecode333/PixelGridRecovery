...
public sealed class ImagePreviewControl : Control
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        if (rect is null)
            return;
        float scale = rect.Value.Width / previewImage.Width;

        var saved = e.Graphics.Save();
        try
        {
            e.Graphics.SetClip(rect.Value);
            DrawCheckerboard(e.Graphics, rect.Value);
            e.Graphics.InterpolationMode = InterpolationMode.NearestNeighbor;
            e.Graphics.PixelOffsetMode = PixelOffsetMode.Half;
            e.Graphics.DrawImage(previewImage, rect.Value, new RectangleF(0, 0, previewImage.Width, previewImage.Height), GraphicsUnit.Pixel);
            e.Graphics.PixelOffsetMode = PixelOffsetMode.Default;
            if (showGrid && grid is { CellWidth: > 0, CellHeight: > 0 })
                DrawGrid(e.Graphics, rect.Value, scale, grid);
        }
        finally { e.Graphics.Restore(saved); }
        using var border = new Pen(Color.FromArgb(110, 116, 125));
        e.Graphics.DrawRectangle(border, rect.Value.X, rect.Value.Y, rect.Value.Width, rect.Value.Height);
    }

    public bool TryGetImagePixel(Point clientPoint, out Point imagePixel)
    {
        imagePixel = default;
        var rect = GetImageRectangle();
        if (previewImage is null || rect is null || clientPoint.X < rect.Value.Left || clientPoint.X >= rect.Value.Right
            || clientPoint.Y < rect.Value.Top || clientPoint.Y >= rect.Value.Bottom)
            return false;
        imagePixel = new Point(
            Math.Min(previewImage.Width - 1, (int)((clientPoint.X - rect.Value.Left) / rect.Value.Width * previewImage.Width)),
            Math.Min(previewImage.Height - 1, (int)((clientPoint.Y - rect.Value.Top) / rect.Value.Height * previewImage.Height)));
        return true;
    }

    internal RectangleF? GetImageRectangle()
    {
        if (previewImage is null) return null;
        ...
        return new RectangleF((ClientSize.Width - previewImage.Width * scale) / 2,
